...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        a=len(X)
        b=len((self.params['W']).transpose())
        forward_output=np.zeros((a,b))
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        o=np.ones(X.shape)
        backward_output=o-np.multiply(X,X)
        ####################################################################################################
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ######################################################################################
            grad_x = model['L1'].backward(x, grad_a2)
